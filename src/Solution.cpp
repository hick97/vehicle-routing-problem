...
Solution Solution::swapping_random_nodes(Solution &sol)
{
  ...
  int random_route = (rand() % (solCopy.routes.size()));

  // Choosing two position in random route to swap
  int first_pos = rand() % (solCopy.routes[random_route].size() - 2) + 1;
  int second_pos = rand() % (solCopy.routes[random_route].size() - 2) + 1;

  // Getting elements
  unsigned int first_element = solCopy.routes[random_route][first_pos];
  unsigned int second_element = solCopy.routes[random_route][second_pos];
  /*
